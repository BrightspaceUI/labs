...
describe('community link', () => {
	Object.keys(langArticleMap).map((langCode) => {
		it(`Should have the right link for each lang code ${langCode}`, async() => {
			...
			const communityLink = await fixture(html`<d2l-labs-link-community article-map=${JSON.stringify(langArticleMap)}></d2l-labs-link-community>`);
			// eslint-disable-next-line no-console
			expect(communityLink.href).to.equal(langUrlMap[langCode]);
		});
	});
	...
	it("Should default to english if the lang code doesn't match", async() => {
		...
		const communityLink = await fixture(html`<d2l-labs-link-community article-map=${JSON.stringify(langArticleMap)}></d2l-labs-link-community>`);
		// eslint-disable-next-line no-console
		expect(communityLink.href).to.equal(langUrlMap['en']);
	});
});
